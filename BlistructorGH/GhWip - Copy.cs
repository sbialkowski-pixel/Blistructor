...
namespace BlistructorWip
{
    ...
    public class Script_Instance : GH_ScriptInstance
    {
        ...
        private void RunScript(List<Curve> cutter, Curve blistr, Curve circle, ref object A, ref object B)
        {
            // <Custom code>
            //List<PolylineCurve> goodCutters = new List<PolylineCurve>();
            //foreach (Polyline pline in cutter)
            //{
            //    goodCutters.Add(new PolylineCurve(pline));
            //}
            //A = Geometry.SplitRegion(blistr, cutter);
            Tuple<List<Curve>, List<Curve>> result = Geometry.TrimWithRegions(blistr, cutter);
            A = result.Item1;
            B = result.Item2;
            //B = new PolylineCurve(blistr).Domain;

            //  A = VerifyPartialPath(goodCutters, blistr, circle, circle);
            // </Custom code>
        ...
        public PolylineCurve VerifyPartialPath(List<LineCurve> cutters, Curve blister, Curve orientationCircle, Curve pillOffset)
        {

            List<CurveIntersections> intersectionsData = new List<CurveIntersections>();
            List<Point3d> intersectionPoints = new List<Point3d>();
            List<Curve> blister_parts = new List<Curve>();
            ...
            // Overlap last vert to get cLosed pLine
            List<Point3d> pLinePoints = sortedInterPoints.ToList();
            // If ther is only 2 points, return nothing
            if (pLinePoints.Count < 3) { return null; }

            pLinePoints.Add(sortedInterPoints[0]);
            // Generate Pline...
            PolylineCurve pLine = new PolylineCurve(pLinePoints);
            //Check if pLine dosent intersect with pill
            CurveIntersections inter2 = Intersection.CurveCurve(pLine, pillOffset, Setups.IntersectionTolerance, Setups.OverlapTolerance);
            if (inter2.Count == 0)
            {
                return pLine;
            }
                ...
                List<Curve> limiters = new List<Curve>
        {
          pillOffset
            ...
            public CutData VerifyPartialPath(List<LineCurve> cutters, Curve blister)
            {
